...
trace    = [];
Hfree    = zeros(n);
clamp    = @(x) max(lower, min(upper, x)); %夹逼函数
% initial state
...
x(~isfinite(x)) = 0; %定哪些数组元素为有限，防止出现NaN
% options
if nargin > 5
...
for iter = 1:maxIter
    ...
    % check relative improvement 如果det_cost/cost < minimum relative improvement，说明收敛了 result = 4
    ...
    grad_clamped   = g  + H*(x.*clamped);                           %可以修改为H*x，那么第120行就不必减去x(free)了
    search         = zeros(n,1);
    search(free)   = -Hfree\(Hfree'\grad_clamped(free)) - x(free);  %公式（16）后面的两个公式
    ...
    vc    = xc'*g + 0.5*xc'*H*xc;
    while (vc - oldvalue)/(step*sdotg) < Armijo  %可以对比增加wolfe准则的收敛性
        step  = step*stepDec;
        nstep = nstep+1;
		xc    = clamp(x+step*search);
        vc    = xc'*g + 0.5*xc'*H*xc;
