...
T       = 500;              % horizon
x0      = [1;1;pi*3/2;0];   % initial state 初始位置
...
figure(9);
set(gcf,'name','car parking','Menu','none','NumberT','off')   % gcf - 当前图窗的句柄; menu off不显示菜单条目 'Menu' figure则显示；  NumberT是否显示图号
set(gca,'xlim',[-4 4],'ylim',[-4 4],'DataAspectRatio',[1 1 1])% gca - 当前坐标区或图 'DataAspectRatio'等比例绘制曲线
grid on
box on
...
set(handles, {'facecolor','edgecolor'}, [fcolor ecolor])      %虚化
% prepare and install trajectory visualization callback
line_handle = line([0 0],[0 0],'color','b','linewidth',2);    %画出每一步优化的轨迹结果 line - 创建基本线条   此 MATLAB 函数 使用向量 x 和 y 中的数据在当前坐标区中绘制线条。如果 x 和 y 中有一个    是矩阵或两者都是矩阵，则 line 将绘制多个线条。与 plot 函数不同，line 会向当前坐标区添加线条，而不删除其他图形对象或重置坐标区属性。
...
%           [ x ]      [x_k+ det_T*cos(yaw)      ]  %
%           [ y ]      [y_k+ det_T*sin(yaw)      ]  %
%  x_k+1 =  [yaw]   =  [yaw_k + asin(sin(w).*f/d)]  %
%           [ v ]      [v_k +det_T*a             ]  %
% === states and controls:
...
% control cost 控制量cost
lu    = cu*u.^2;
% final cost  终端cost
if any(final)
   llf      = cf*sabs(x(:,final),pf);  %终点为[0 0 0 0]→【X,Y,YAW,V】
   lf       = double(final);
   lf(final)= llf;
else
   lf    = 0;
...
c     = lu + lx + lf;
function y = sabs(x,p)
% smooth absolute-value function (a.k.a pseudo-Huber) %Pseudo-Huber损失函数是MAE函数的鲁棒性和 MSE的实际现有解的结合体。该损失函数还有一个附加参数 delta，主要用于控制函数从二次到线性切换的位置。此外该参数还用于剪裁梯度值，从而可以限制异常值的影响。
...
if nargout == 2
    ...
    if full_DDP
        ...
        JJ      = reshape(JJ, [4 6 size(J,2,3)]); %%[4 6 size(J)]修改为[4 6 size(J,2,3)]
...
% simple finite-difference derivatives 有限差分法
% assumes the function fun() is vectorized
if nargin < 3
    h = 2^-17;
end
[n, K]  = size(x);
H       = [zeros(n,1) h*eye(n)]; %从二维的角度看，H的维度是6x7；从三维的角度看，H的维度是6x7x1
H       = permute(H, [1 3 2]);   %由之前的维度调整为[1 3 2],那么最终的结果就是6x1x7
X       = pp(x, H);
X       = reshape(X, n, K*(n+1));% [x1_1~K  x_1~K+h  x1_1~K   ... x1_1~K  ]
                                 % [x2_1~K  x2_1~K   x2_1~K+h ... x2_1~K  ]
                                                ...
                                 % [xn_1~K  xn_1~K   xn_1~K   ... xn_1~K+h]  %  x1~xn为 【x u】' ;x1_1~K为状态量x1的1~K的预测量 
                                 %    0       1        2      ...  n 
Y       = fun(X);
m       = numel(Y)/(K*(n+1));
Y       = reshape(Y, m, K, n+1);
...
function c = pp(a,b)  % 对数组 A 和 B 应用函数句柄 fun 指定的按元素二元运算
c = bsxfun(@plus,a,b);  % a和b对应元素相加
function c = tt(a,b)
c = bsxfun(@times,a,b); % a和b对应元素相乘