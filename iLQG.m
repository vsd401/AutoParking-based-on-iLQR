...
%
% lambda - the final value of the regularization parameter 正则参数？？
%
...
verbosity = Op.print;  %打印日志
switch numel(Op.lims)  %numel返回元素数目
    case 0
    case 2*m
...
% --- initial trajectory 轨迹初始化
if size(x0,2) == 1
    diverge = true;
    for alpha = Op.Alpha
        [x,un,cost]  = forward_pass(x0(:,1),alpha*u,[],[],[],1,DYNCST,Op.lims,[]); %第五个输入量是k，第三个输入量是K
...
for iter = 1:Op.maxIter
    ...
    %====== STEP 1: differentiate dynamics and cost along new trajectory   %沿着最新状态轨迹，跟新微分量和cost
    if flgChange
        t_diff = tic;
        [~,~,fx,fu,fxx,fxu,fuu,cx,cu,cxx,cxu,cuu]   = DYNCST(x, [u nan(m,1)], 1:N+1); %前两个分别为car dynamics and cost
    ...
    while ~backPassDone
        ...
        t_back   = tic;
        [diverge, Vx, Vxx, l, L, dV] = back_pass(cx,cu,cxx,cxu,cuu,fx,fu,fxx,fxu,fuu,lambda,Op.regType,Op.lims,u);%第四个输出量是k，第五个是K
    ...
    if backPassDone
        ...
        if Op.parallel  % parallel line-search
            [xnew,unew,costnew] = forward_pass(x0 ,u, L, x(:,1:N), l, Op.Alpha, DYNCST,Op.lims,Op.diffFn); %第五个输入量是k，第三个输入量是K
...
function [xnew,unew,cnew] = forward_pass(x0,u,L,x,du,Alpha,DYNCST,lims,diff) %第五个输入量是k，第三个输入量是K
...
for i = 1:N
    ...
    if ~isempty(du)                                %    \
        unew(:,:,i) = unew(:,:,i) + du(:,i)*Alpha; %    |
                                                   ...
                                                   %    |
    if ~isempty(L)                                 %    |
        if ~isempty(diff)                          %    |> det_u = K*det_x + k,这里du就是k
    ...
    if ~isempty(lims)
        unew(:,:,i) = min(lims(:,2*K1), max(lims(:,1*K1), unew(:,:,i))); %不等式夹逼再次确认
    end
    [xnew(:,:,i+1), cnew(:,:,i)]  = DYNCST(xnew(:,:,i), unew(:,:,i), i*K1);
end
[~, cnew(:,:,N+1)] = DYNCST(xnew(:,:,N+1),nan(m,K,1),i); %cnew是cost
...
for i = N-1:-1:1
    ...
    if nargin < 13 || isempty(lims) || lims(1,1) > lims(1,2)
        ...
        % find control law
        kK = -R\(R'\[Qu Qux_reg]); %相当于 inv(R'R)*Qu
        k_i = kK(:,1);      %相当于d =-inv(Quu)*Qu
        K_i = kK(:,2:n+1);  %相当于K =-inv(Quu)*Qux
        ...
        if any(free)
            ...
            K_i(free,:)   = Lfree;                 %相当于K =-inv(Quu)*Qux
        end
        
    end
    
    % update cost-to-go approximation
    dV          = dV + [k_i'*Qu  .5*k_i'*Quu*k_i];         %残余增量page12 in ddp.pdf
...
if figures ~= 0  && ( mod(mT,figures) == 0 || init == 2 )
    ...
    line(1:N,cost,'linewidth',4,'color',.5*[1 1 1]); %粗线为代价结果，刻度为右侧标尺
    ax2 = axes('Position',get(ax1,'Position'));
    plot((1:N),x','linewidth',2); % 细线为x状态量结果，刻度为默认左侧标尺
    set(ax2,'xlim',[1 N],'Ygrid','on','YMinorGrid','off','color','none');
    set(ax1,'Position',get(ax2,'Position'));
    double_title(ax1,ax2,'state','running cost')  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
    
    axL = subplot(2,2,3);
    CO = get(axL,'colororder');
    set(axL,'nextplot','replacechildren','colororder',CO(1:min(n,7),:))
    Lp = reshape(permute(L,[2 1 3]), [nL*m N-1])';  %反馈增益
    ...
    double_title(axu,axL,'controls','gains')  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%控制量以及反馈增益K
    xlabel 'timesteps'
    ...
    set(ax1,'Position',get(ax2,'Position'));
    double_title(ax1,ax2,'convergence trace','total cost') %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
    
    subplot(2,2,4);
    plot(T,[trace(T).reduc_ratio]','.-','linewidth',2);
    title 'actual/expected reduction ratio'               %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%5
